...
>>>>>>> 6d13f5374000ae59c6aaae8765b47805b9a3494d
    ...
    function applyFilters() {
        ...
        tableRows.forEach(row => {
            ...
            if (showRow && paymentFilter) {
                const paymentStatusCell = row.cells[5];
                const paymentStatusText = paymentStatusCell.textContent.trim().toLowerCase();
                if (!paymentStatusText.includes(paymentFilter)) {
                    showRow = false;
                }
            }
            ...
            if (showRow && filterValue && !['price_high', 'price_low'].includes(filterValue)) {
                ...
                const orderStatusText = orderStatusCell.textContent.trim().toLowerCase();
                const normalizedFilterValue = filterValue.replace(/_/g, ' ');
                if (!orderStatusText.includes(normalizedFilterValue)) {
                    showRow = false;
                }
            }
                ...
                if (!isValidDate(rowDate)) {
                    ...
                    switch (dateFilter) {
                        ...
                        case 'last_week':
                            ...
                            if (rowDate < weekAgo || rowDate > today) showRow = false;
                            break;
                        case 'last_month':
                            const monthAgo = new Date(today);
                            monthAgo.setDate(monthAgo.getDate() - 30);
                            if (rowDate < monthAgo || rowDate > today) showRow = false;
                            break;
                        case 'this_month':
                            if (rowDate.getMonth() !== today.getMonth() ||
                                rowDate.getFullYear() !== today.getFullYear()) showRow = false;
                            break;
                        case 'last_3months':
                            const threeMonthsAgo = new Date(today);
                            threeMonthsAgo.setMonth(threeMonthsAgo.getMonth() - 3);
                            if (rowDate < threeMonthsAgo || rowDate > today) showRow = false;
                            break;
                    }
                }
            }
        ...
        // Handle price sorting if selected
    ...
    function exportRowToExcel(button) {
        ...
        // Create data array for the single row
        const rowData = {
            'Service Name': row.cells[0].textContent.trim(),
            'Date': row.cells[1].textContent.trim(),
            'Branch Name': row.cells[2].textContent.trim(),
            'Payment Category': row.cells[3].textContent.trim(),
            'Service Cost': row.cells[4].textContent.trim(),
            'Payment Status': row.cells[5].querySelector('.badge').textContent.trim(),
            'Order Status': row.cells[6].querySelector('.badge').textContent.trim()
        };
...
<<<<<<< HEAD
        ...
        const serviceName = rowData['Service Name'].replace(/[^a-z0-9]/gi, '_').toLowerCase();
